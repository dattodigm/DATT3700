...
extern uint8_t getControlMode();

const uint8_t kModeTrack = 1;
...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  // In TRACK mode, iris position follows OSC coordinates directly.
  if (getControlMode() == kModeTrack) {
    float nx = 0.5f, ny = 0.5f;
    uint8_t blendPct = 100;
    if (isOscTrackingActive()) getOscTrackingNorm(nx, ny, blendPct);

    eyeX = constrain((int16_t)round(nx * 1023.0f), 0, 1023);
    eyeY = constrain((int16_t)round(ny * 1023.0f), 0, 1023);
  }
